...
// ───────────────────────── Rave Distribution ─────────────────────────
// Line shape (after PDF text extraction, whitespace-collapsed):
//   <lineNo> <UPC> <DESCRIPTION possibly wrapping> <qty> $<sold> $<tax> $<amount>
function parseRave(text) {
  ...
  // Price tail: qty  $sold  $tax  $amount
  const priceTail = /(\d+(?:\.\d+)?)\s+\$([\d,]+\.\d{2})\s+\$([\d,]+\.\d{2})\s+\$([\d,]+\.\d{2})\s*$/;
  // Line starts with "<lineNo> <10-14 digit UPC>". Some lines have an extra internal
  // number prefix (e.g. "2 697 791684114427 ...") so permit 1–2 leading groups.
  const itemHead = /^(\d+)\s+(?:\d+\s+)?(\d{10,14})\s+/;
  const stopWord = /^(Total|Subtotal|Grand|Previous|General|Return|Page|Kentucky|Thank|Date\s*&)/i;

  const items = [];
  for (let i = 0; i < lines.length; i++) {
    if (!itemHead.test(lines[i])) continue;

    let combined = lines[i];
    let j = i;
    while (!priceTail.test(combined) && j + 1 < lines.length) {
      j++;
      if (stopWord.test(lines[j])) break;
      if (itemHead.test(lines[j])) break; // next item started; bail
      combined += ' ' + lines[j];
    }
    const tail = priceTail.exec(combined);
    if (!tail) continue;

    const head = itemHead.exec(combined);
    const upc  = head[2];
    const qty  = parseFloat(tail[1]);
    const sold = parseMoney(tail[2]);
    const amt  = parseMoney(tail[4]);

    // Description is what's between the head and the price tail.
    let desc = combined.slice(head[0].length, combined.length - tail[0].length).trim();
    // Rave often wraps the final token of the description onto the next line
    // ("... | WILD" → next line: "WATERMELON"). If the next line looks like
    // free-form text (not a new item, not a summary keyword), absorb it.
    if (j + 1 < lines.length && !itemHead.test(lines[j + 1]) && !stopWord.test(lines[j + 1])
        && !priceTail.test(lines[j + 1]) && lines[j + 1].length < 60) {
      j++;
      desc += ' ' + lines[j];
    }
    // Rave's PDF sometimes duplicates the description — collapse "X X" → "X".
    if (desc.length >= 10) {
...
async function parsePdfBuffer(buffer) {
  // pdf-parse v2 exposes a CJS default that is the parse function itself.

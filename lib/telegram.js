...
export function buildSyncSummaryMessage(results, targetDate, shortOverAlerts) {
  ...
  for (const r of results) {
    ...
    const nonTax = d.non_tax_sales || 0;
    const total = d.total_sales || (net + nonTax);
    ...
    msg += `━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `💰 Gross: <b>${formatCurrency(gross)}</b> (incl. tax)\n`;
    msg += `📊 Total: <b>${formatCurrency(total)}</b>\n`;
    if (nonTax > 0) msg += `   Taxable: ${formatCurrency(net)}  Non-tax: ${formatCurrency(nonTax)}\n`;
  ...
  if (withData.length > 1) {
    ...
    msg += `📊 Total: <b>${formatCurrency(totals.net)}</b>\n`;

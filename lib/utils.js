...
export const dateLabel = (d) => new Date(d + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
export const weekLabel = (w) => new Date(w + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
// Week keys are Mondays (YYYY-MM-DD). Render as a Mon–Sun range like "Mar 30 – Apr 5".
export const weekRangeLabel = (w) => {
  const start = new Date(w + 'T12:00:00');
  const end = new Date(start); end.setDate(start.getDate() + 6);
  const f = (d) => d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
  return `${f(start)} – ${f(end)}`;
};
// Snap a YYYY-MM-DD date (or Date) to the Monday of its Mon–Sun week.
// Parses at local noon and formats from local parts — otherwise UTC
// rollover drifts the key to Tuesday in negative-offset timezones and
// rows end up in their own column next to the seeded Monday bucket.
export const startOfWeekMonday = (d) => {
  ...
  const clean = (typeof d === 'string' ? d : iso(new Date(d))).split('T')[0];
  const dd = new Date(clean + 'T12:00:00');
  const day = dd.getDay();
...
// Store names are stored fully qualified ("7s Vape Love - Reno"), which is
// far too long for a column header or a chip on a phone — and truncating it
// leaves every store reading "7s Vape Love …". The part after the dash is
// the bit that actually identifies the store.
export const storeShortName = (name) => String(name || '').split(' - ').pop().trim() || String(name || '');

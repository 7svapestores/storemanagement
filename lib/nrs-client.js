...
// For stores with a manual R2 cash register, NRS only sees R1 activity.
// R2 Net equals R1 Canceled Basket (cash canceled in R1 is re-rung on R2).
// R2 Safe Drop stays 0 from NRS since the manual register isn't tracked; the
// cashier can correct it through the edit form if needed.
export function applyRegister2AutoSync(parsed, hasRegister2) {
  if (!hasRegister2) return parsed;
  const cb = Number(parsed.r1_canceled_basket || 0);
  return {
    ...parsed,
    r2_net: cb,
    r2_gross: cb,
...
export function parseNRSStatsToDailySales(nrsData, storeId, date) {
  ...
  // Non-taxable: byday.sales is total net (taxable+nontax), taxable_amt.amt is just taxable
  const totalNetFromByday = cents(byday.sales);
  const taxableOnly = cents(taxableAmt.amt);
  const nonTaxSales = totalNetFromByday > taxableOnly ? parseFloat((totalNetFromByday - taxableOnly).toFixed(2)) : 0;
  console.log('[nrs/parse] nonTax calc: byday.sales=', byday.sales, 'taxable_amt=', taxableAmt.amt, 'nonTax=', nonTaxSales);
  ...
  const grossSales = cents(payamts.total);
  // Use taxableOnly when available; fall back to totalNetFromByday only if taxableOnly is null/undefined
  const netTaxable = (taxableOnly != null && taxableOnly > 0) ? taxableOnly : totalNetFromByday;
  // total_sales = taxable net + non-taxable (NEVER gross)
  const totalSales = parseFloat((netTaxable + nonTaxSales).toFixed(2));
  console.log('[nrs/parse] sales: gross=', grossSales, 'netTaxable=', netTaxable, 'nonTax=', nonTaxSales, 'total=', totalSales);

...
export async function fetchNRSDailyStats(nrsStoreId, date) {
  ...
  // NRS wraps the payload in { res: { rc, msg }, data: { ... } }
  if (json.data) {
    console.log('[nrs/fetch] data keys:', Object.keys(json.data));
    return { _raw: json, ...json.data };
  }
  return { _raw: json, ...json };
}

export function parseNRSStatsToDailySales(nrsData, storeId, date) {
  const { _raw, ...d } = nrsData || {};
  ...
  return {
    ...
    ai_extracted_data: _raw || nrsData,
  };

...
export async function validateNRSAuth() {
  if (!NRS_TOKEN) return false;
  try {
    const res = await fetch(`${NRS_BASE}/auth/validate`, { headers: NRS_HEADERS });
    return res.ok;
  } catch {
    return false;
  }
...
export async function fetchNRSDailyStats(nrsStoreId, date) {
  ...
  const url = `${NRS_BASE}/pcrhist/${nrsStoreId}/stats/day/${date}/${date}?elmer_id=0`;
  const res = await fetch(url, {
    headers: {
      ...NRS_HEADERS,
      'Cookie': `user=${NRS_TOKEN}`,
    },
  });
  if (!res.ok) {

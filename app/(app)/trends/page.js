...
import { fmt, fK, weekRangeLabel, startOfWeekMonday } from '@/lib/utils';
...
export default function TrendsPage() {
  ...
  useEffect(() => { (async () => {
    ...
    const map = {};
    sales?.forEach(s => { const w = startOfWeekMonday(s.date); map[w] = {...(map[w]||{sales:0,purchases:0}), sales: (map[w]?.sales||0)+(s.total_sales||0)}; });
    purch?.forEach(p => { const w = startOfWeekMonday(p.week_of); map[w] = {...(map[w]||{sales:0,purchases:0}), purchases: (map[w]?.purchases||0)+(p.total_cost||0)}; });
